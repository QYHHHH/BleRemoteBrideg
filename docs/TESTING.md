...
| 中英切换 | 默认中文；静态标记、JS 渲染的卡片、运行期状态串、下拉选项都跟着语言走；切到英文能还原作者原文（证明缓存生效、词典无需存两份） |
...
**恢复出厂的效果**（当时还有网页密码，现已去掉，见 [`AUTH.md`](AUTH.md)）：
BOOT 长按 5 秒 → `settings::clearAll()` 用 `s_prefs.clear()` 清空整个命名空间
（Wi-Fi 凭据、按键映射、状态缓存），配合 `ble_bonds::removeAll()` 清蓝牙配对 →
完整出厂状态。
...
| 35 | 设备信息 | 选口后页面读到的设备信息 | 名称 `Mi Remote Bridge`、固件 `MiRemoteBridge` / 当前 `config.h` 里的 `BRIDGE_FW_VERSION`、芯片字段 `esp32c3/esp32-c3` | ☐ |
| 36 | **控制线冲突（最要紧）** | 用网页保持端口连接 30 秒以上，什么都不做（**不是**普通串口软件，后者见下方注） | 板子**不重启、不进下载模式、不恢复出厂**——浏览器落在 `(1,1)`，两根线都不碰 GPIO9 | ☐ |
...
> **第 36 项的正确模型（2026-09-17 上机实测重写，此前两轮结论都是错的）。**
>
> 本板是**经典双三极管自动下载电路**，**DTR 与 RTS 成对起作用**，不是各管一根。
> 判据用的是干净的两条：HTTP `/api/status` 的 `uptimeMs` 有没有归零（读它不碰串口），
> 以及串口日志里有没有 `[BUTTON ] key pressed`。脚本在 `build/`：
> `line_probe.py`（逐根线隔离，结论以此为准）/ `open_probe.py`（open 是否复位）/
> `rts_probe.py`。
>
> 逐根线隔离的结果（`line_probe.py`，每一步都发 `status` 问板子在不在）：
>
> | (DTR, RTS) | 板子应答 | `key pressed` |
> | --- | --- | --- |
> | `(0,0)` | 应答 | 无 |
> | `(1,0)` | 应答 | **有** |
> | `(1,1)` | 应答 | 无 |
> | `(0,1)` | **无应答**（EN 低） | 无 |
>
> 即：EN 拉低要 `(0,1)`，GPIO9 拉低要 `(1,0)`，`(0,0)` 与 `(1,1)` 都正常。
>
> | (DTR, RTS) | 后果 | 取证 |
> | --- | --- | --- |
> | `(0,0)` | 不复位 | open 保持 3 s，`uptimeMs` 69729536 → 69735475 连续增长 |
> | `(1,1)` | 正常，GPIO9 不动 | 置位 1.5 s（跨一次启动）零 `key pressed` |
> | `(0,1)` | **EN 低 = 板子被按在复位里** | 开端口 HTTP 立刻全灭，释放才启动 |
> | `(1,0)` | **GPIO9 低 = 按住 BOOT** | ~100 ms 后 `[BUTTON ] key pressed` |
>
> **第 36 项现在有实测依据，不再是"唯一无法在代码里证明的风险点"**：浏览器打开
> 端口落在 `(1,1)`。Chromium 在一次 `SetCommState` 里同时置
> `fDtrControl = DTR_CONTROL_ENABLE` 与 `fRtsControl = RTS_CONTROL_ENABLE`
> （`services/device/serial/serial_io_handler_win.cc`），从 `(0,0)` **原子跳到
> `(1,1)`，不经过 `(0,1)`**，EN 上没有任何脉冲。所以配网页本来就安全，不必依赖
> "客户端会不会在 5 秒内发第一帧"。
>
> 由此**固件里那套"Improv 会话期间不触发 BOOT"的防护已被删除**（2026-09-17，
> `sessionActive()` 连同 `reset_button.cpp` 里的两处判断一起去掉，编译 43%）。
> 原因见本节末尾。
>
> **两条被废弃的旧结论**（别再照着查）：
>
> 1. ~~"打开串口会复位板子"~~ —— 要拆开说。复位只发生在控制线**经过 `(0,1)`**
>    的那一刻，本项目有两处：
>    - **pyserial 的 `open()` 本身复位一次**：它先置 RTS 再置 DTR，路径是
>      `(0,0)→(0,1)→(1,1)`。实测（`tests/tools/open_probe.py`）：不预置控制线开 →
>      出现启动横幅；预置成 `(0,0)` 再开 → 没有横幅、板子照常应答。
>    - `build/ser.py` / `serlog.py` 随后的 `s.dtr = False` / `s.rts = False`
>      **再复位一次**，所以默认是**两次**（刻意如此，保证拿到启动横幅）。
>      **`SER_NORESET=1` 实测降到 0 次。**
>    - **Chromium 与 .NET 都不复位**：Chrome 一次 `SetCommState` 同时置两根，
>      不经过 `(0,1)`；.NET `SerialPort` 默认 `(0,0)`。这就是"用户从来没因为
>      串口重置过"的原因。
> 2. ~~"`DtrEnable=true` 等于按住 BOOT"~~ —— **只在 RTS 同时为低时成立**。
>    2026-09-16 那次记为"只差 DTR 一个变量"的隔离复现，RTS 停在 .NET
>    `SerialPort` 的默认值 `$false`，所以实际测的是 `(1,0)`，不是"DTR 单独"。
>
> **通用判据**：打开工具后看日志里有没有 `[BUTTON ] key pressed`。有 → 说明
> **RTS 是低的**（`(1,0)`），**不是**"DTR 被置位"；把 RTS 也置位、或把 DTR 放开，
> 任一即可解除。想绝对安全就把**两根都设 `$false`**（`(0,0)`）——`scripts/monitor.ps1`
> 与 `tests/tools/board_auth.py` 都已经是这个组合。
>
> **另一个必须知道的坑**：`close()` **不释放控制线**。实测 RTS 置位后关端口，板子
> 仍被按在复位里，直到下一次 open 显式 `rts=False` 才起来。脚本收尾必须显式
> `dtr=False; rts=False`，不能只靠 `close()`。
>
> **第 36 项的固件防护已于 2026-09-17 拆除**（用户决定）。原来的实现是：
> `reset_button.cpp` 用 `improv_serial::sessionActive()`（= 5 分钟内收到过 Improv
> 帧）判定"这个 pin 属于配网客户端"，命中就吞掉按键。但按上表浏览器根本不会拉低
> GPIO9，所以它**只可能对真按键生效**，后果是：
>
> - 用过配网页后 5 分钟内**短按 BOOT 被吞掉**，日志打
>   `short press ignored: an Improv provisioning client is using this port`；
> - 同窗口内**长按 5 秒不恢复出厂** —— 而那是 `RECOVERY.md` 里的卡死恢复路径。
>
> 它想防的场景（某个工具把线停在 `(1,0)`）本来就防不住：那种工具不发 Improv 帧，
> `sessionActive()` 恒为假。**既无效又误伤，所以整套机制（函数 + 状态 + 两处判断）
> 一并删除。** 现在的语义简单了：**引脚为低就是手指，短按必开窗口、满 5 秒必恢复
> 出厂。**
>
> 验收这一项时顺带确认：用配网页之后再短按 BOOT，窗口应当照常刷新；
> 长按 5 秒应当照常恢复出厂。
>
> 另见 HANDOFF §5 与 [`PITFALLS.md`](PITFALLS.md)。
