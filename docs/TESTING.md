...
> 第 36 项原本是这一轮唯一**无法在代码里证明**的风险点。本板 USB 串口的 DTR 接在
> GPIO9（BOOT）上，而浏览器打开串口时会置位 DTR（Web Serial 规范：`open()` 之后
> DTR 与 RTS 均置位，页面没有 API 能关掉），这与"按住 BOOT"在 GPIO9 上产生的电平
> 完全一样。固件的防护（收到过 Improv 帧后本次按住不再触发恢复出厂）能不能生效，
> 取决于**客户端会不会在 5 秒内发出第一帧**。
>
> 已核对公开客户端的实现（`esp-web-tools@10.4.0` 的 `install-dialog` 构建产物，
> 内含官方 Improv JS 串口客户端）：`initialize()` 里**立刻**发一次 `_sendRPC(2, [])`，
> 同时挂 `setInterval(..., 1000)` **每秒重发**，直到设备回报状态才清掉。
