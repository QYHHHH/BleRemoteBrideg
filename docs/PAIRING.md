...
[   1450][HID    ] HID peripheral up: name="Mi Remote Bridge" report-map 72 bytes, host stack nimble
...
3. 按键效果要**在有输入焦点的地方看**（备忘录、任何文本框）：方向键移动光标、
   确定回车、菜单打空格、音量键调音量。组合键（Win+Tab、Ctrl+Win）在 iOS 上按
...
| **D4 双闪 / 网页提示"遥控器配对信息已失效"** | 上游密钥已确认失效，固件**已停止自动重连**（`status` 里 `rc003 repair : required`）。让遥控器进入配对广播（§1.1），再到网页"附近设备"里**明确选中它**——固件会删掉当前活动槽的旧 Bond 并重配，已学按键与槽位映射保留。详见 `RECOVERY.md` §2.1 ② |
...
三个槽位均使用“添加设备 → 搜索 → 选择名称/地址”的流程，不限制品牌。点击槽位切换当前遥控器；同时只连接一个遥控器，被控端蓝牙保持连接。未激活遥控器的配对安全记录归档到 NVS，切回时恢复，无需每次重新配对。

- 槽位 01 使用 RC003 固定的 13 键模板，仅转发该模板支持的键码，多余键码丢弃。
- 槽位 02、03 动态发现按键，每槽最多 128 个，可命名、删除并配置快捷键；未配置的键不转发。兼容范围取决于现有 HID 报告解析器，并不承诺任意蓝牙遥控器均可用。
