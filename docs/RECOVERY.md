...
**短按（< 3 秒松开）不是无操作**——它会打开/续期 Web 配置页的 30 分钟窗口
（详见 [`WEB-UI.md`](WEB-UI.md)、[`AUTH.md`](AUTH.md)），这是这颗键平时最常用的
功能。只有**持续按住到 5 秒**才会恢复出厂；3–5 秒之间松开则两件事都不做，
只留一条日志。串口 `factory` 命令与长按满 5 秒走同一条代码路径，效果完全相同。
