...
做的事：删除当前连接主机的 Bond 记录 → 主动断开连接 → 重新广播。
之后 Windows 需要重新配对。

**对端也要清吗？** 建议是。Windows 里 **设置 → 蓝牙和其他设备 → `Mi Remote Bridge` → 删除设备**。
...
| **Windows 侧描述符缓存错乱**（显示未知设备、按键错位） | ① `forget win` ② Windows 删除设备 ③ 重新配对 ④ 确认日志出现 `report 1 notifications ENABLED` |
| **Windows 重启后连不上** | 通常 Windows 会自动重连。若没连上：`forget win` + 关/开 Windows 蓝牙。**不需要**清 RC003 的 Bond |
