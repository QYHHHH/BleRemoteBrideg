...
// Latched by a failed downstream authentication, cleared by a successful one.
// See hid_server.h for why advertising keeps running while it is set.
bool s_hostRepairRequired = false;
...
void setHostRepairRequired(bool required) {
  if (required == s_hostRepairRequired) return;
  s_hostRepairRequired = required;
  if (required) {
    BR_LOGW(kTagHost, "stored host key rejected; re-add \"%s\" in Windows to pair again", s_deviceName.c_str());
  } else {
    BR_LOGI(kTagHost, "host pairing is valid again");
  }
}

void clearHostRepair() { setHostRepairRequired(false); }

bool hostRepairRequired() { return s_hostRepairRequired; }
