...
function drawSlots(){
 ...
 return '<button class="slot '+(active?'selected':'')+'" data-slot="'+i+'" aria-pressed="'+active+'"><span class="slot-index">槽位 0'+(i+1)+(i===0?' · 小米预设':'')+'</span><strong>'+esc(slotTitle(i))+'</strong><span class="slot-link '+(rc?'linked':'')+'">'+(rc?'● 遥控器已连接':s.address?(active?'○ 正在连接':'○ 未启用'):'○ 未添加设备')+'</span><span class="slot-battery">'+(rc?(S.lastStatus.battery>=0?'电量 '+S.lastStatus.battery+'%':'电量未读取'):esc(s.address)||'点击添加设备')+'</span></button>'}).join('');
