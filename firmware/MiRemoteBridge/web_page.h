...
const storageKey='mrb-three-slot-ui-v1';let slots;
try{slots=JSON.parse(localStorage.getItem(storageKey))}catch(e){}
if(!Array.isArray(slots)||slots.length!==3)slots=[{name:'RC003',bound:true,keys:[]},{name:'遥控器 02',bound:false,keys:[]},{name:'遥控器 03',bound:false,keys:[]}];
function persist(){try{localStorage.setItem(storageKey,JSON.stringify(slots));return true}catch(e){return false}}
function presetKeys(){return KEYS.map(k=>({raw:k[0],name:k[1],action:{...cur(k[0])}}))}
function slotFmt(a){return a.kind===0?'不转发':fmt(a)}
function slotConnected(s,i){return i===0&&S.lastStatus?!!S.lastStatus.remoteConnected:s.bound}
function slotBattery(s,i){return i===0&&S.lastStatus&&typeof S.lastStatus.battery==='number'?S.lastStatus.battery:(s.bound?85:'未知')}
const ICON_CODES=['du','dd','dl','dr','p6','p7','p3','p4'];
const SIM_KEYS=[0x52,0x51,0x50,0x4F,0x80,0x81,0xF1,0x4A,0x90,0x91,0x92];
...
function drawDiscovered(){const s=slots[selectedSlot];
 ...
 if($('simulateBind'))$('simulateBind').onclick=()=>{s.bound=true;persist();drawSlots()};
 if($('simulateKey'))$('simulateKey').onclick=()=>discoverKey(nextSimRaw(s.keys));
 if($('simulateRepeat'))$('simulateRepeat').onclick=()=>{if(s.keys.length)discoverKey(s.keys[s.keys.length-1].raw)};
}
