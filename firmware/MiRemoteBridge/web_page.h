...
function btns(){var L=S.busy||S.load||S.slotBusy;Array.prototype.forEach.call(document.querySelectorAll('[data-r],[data-learned],#resetAll,#uiReset,#pairRemote,#deleteRemote,#repairHost,#simulateBind,#simulateKey,#simulateRepeat,#slotEditor input,#slotEditor select,#slotEditor .m,#slotDelete'),function(e){e.disabled=!S.ok||!S.on||L});Array.prototype.forEach.call(document.querySelectorAll('[data-slot]'),function(e){e.disabled=!S.ok||!S.on||S.load});
...
pill('pR',rc,rc?'遥控器已连接':'遥控器未连接');pill('pH',h,h?'被控蓝牙已连接':paused?'电脑需重新配对':'被控蓝牙未连接');pill('pB',bat!=='未知',bat==='未知'?'电量未知':'电量 '+bat);
...
side.prepend(rack);
const hostRepair=document.createElement('div');hostRepair.className='warn';hostRepair.id='hostRepair';hostRepair.hidden=true;hostRepair.innerHTML='<span><b>电脑配对密钥已失效</b><br>请先在 Windows 蓝牙设置中删除 Mi Remote Bridge，再点击右侧按钮。</span><button id="repairHost">已删除，重新配对</button>';rack.after(hostRepair);
$('repairHost').onclick=async()=>{if(!S.on||S.busy)return;S.busy=true;btns();try{await req('/api/host-pairing','POST');await req('/api/status').then(stat);toast('已开始重新配对，请在 Windows 中添加设备')}catch(e){toast(err(e))}finally{S.busy=false;btns()}};
...
async function slotAction(action,slot){
 if(!S.on||S.load||((S.busy||S.slotBusy)&&!(action==='select'&&slot!==selectedSlot)))return;
