...
function drawSlots(){
 ...
 return '<button class="slot '+(active?'selected':'')+'" data-slot="'+i+'" aria-pressed="'+active+'"><span class="slot-index">槽位 0'+(i+1)+(i===0?' · 小米预设':'')+'</span><strong>'+esc(slotTitle(i))+'</strong><span class="slot-link '+(rc?'linked':'')+'">'+(rc?'● 遥控器已连接':s.address?(active?'○ 正在连接':'○ 未启用'):'○ 未添加设备')+'</span><span class="slot-battery">'+(rc&&S.lastStatus.battery>=0?'电量 '+S.lastStatus.battery+'%':esc(s.address)||'点击添加设备')+'</span></button>'}).join('');
...
async function slotAction(action,slot){
 ...
 try{
  ...
  for(let i=0;i<60;i++){const j=await req('/api/slots');applySlots(j);if(!j.busy){if(j.error)throw Error(j.error);break}if(i===59)throw Error('操作仍在进行，请稍候');await new Promise(r=>setTimeout(r,500))}
