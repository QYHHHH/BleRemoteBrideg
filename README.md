...
- **测试基建**：宿主端模型检查（11098 项断言，含 4000 步随机不变量测试）+
  设备端自检（139 项向量 + 44 项分发仿真），宿主与设备消费同一份期望值向量；
...
| 返回 | `0xF1` | Backspace | 可改 `consumer_back` / `kb_esc` / `kb_alt_left` |
| 上 / 下 / 左 / 右 | `0x52` / `0x51` / `0x50` / `0x4F` | ↑ ↓ ← → | 标准键盘 Usage |
| 确定 | `0x28` | Enter | |
| 主页 | `0x4A` | Win + Tab | 第三方写的 `0x24` 作为同义词保留 |
| 菜单 | `0x65` | 空格 | 同义词 `0x5D` |
| 电视 | `0x35` | F | 同义词 `0xC0` |
| 电源 | `0x66` | Esc | 可改 Alt + F4 / Sleep / Power |
...
| L3 设备端自检 | ✅ 真机：`139 passed / 0 failed` + 分发仿真 `44 passed / 0 failed` |
