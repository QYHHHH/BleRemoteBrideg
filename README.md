...
- **Web 配置界面**：开机自动连上 Wi-Fi 并提供按键映射页面（手机 / 电脑浏览器直接访问），
  逐键绑定电脑快捷键或媒体键，配置持久化；按下遥控器时页面立即高亮（实时反馈）。
...
**实时响应（事件通道，不是轮询）**：页面把一个请求**挂在设备上**
（长轮询 `GET /api/events?since=<计数>`），设备一有按键**立即应答**——实测
**23 ms**（松开 25 ms）。请求携带事件计数，因此**再短的按压也不会丢**；
连接状态与电量另走 3 秒一次的轻量刷新。

> 为什么不用 SSE / WebSocket：本服务器**一次只服务一个请求**，一条长期流式
> 连接会把页面自己的保存请求挡在门外。长轮询同样由事件驱动，却只占一个
> 普通请求的位置——页面在提交保存前会先撤下它，保存完立刻续上。
>
