...
- **双状态指示灯**：板上 D5 / D4 分别指示电脑链路与遥控器链路，呼吸 = 等待、
  快闪 = 连接中、常亮 = 就绪、双闪 = 这条链路要重新配对；按住遥控器按键时对应灯
  熄灭（现场确认按键已被接收），十分钟无操作后自动熄灭。
...
常亮 = 已就绪，双闪 = 这条链路需要你动手处理**。不接电脑、不看日志，也能一眼
判断桥接器到哪一步了。

| LED | 引脚 | 指示对象 | 呼吸（等待） | 快闪（连接中） | 常亮（已连接） | 双闪（需要处理） |
| --- | --- | --- | --- | --- | --- | --- |
| **D5** | GPIO13 | 电脑 / 手机（下游主机） | 无连接，正在广播 | 已建链路，HID 尚未就绪 | HID 已订阅，按键可用 | 旧密钥已失效，要在 Windows 里删除设备再重新添加 |
| **D4** | GPIO12 | 遥控器（上游 RC003） | 搜索中（空闲 / 扫描 / 退避） | 正在连接或发现服务 | 已订阅，可转发 | 遥控器配对信息已失效，要重新配对 |

**双闪**：亮 120 ms → 灭 120 ms → 亮 120 ms → 灭约 1.24 s，约 1.6 秒一轮。
与 2 秒一轮的呼吸、90 ms 半周期的快闪刻意拉开节奏，不会被误读成"等待"或
"正在连接"。

**遥控器按键反馈**：按住任意遥控器按键时 **D4 熄灭**，松开恢复——这是"桥接器
确实收到了这个键"的现场确认（用的是与串口 `status`、Web UI 相同的按键状态）。

**十分钟后自动熄灭**：两条链路都十分钟没有活动时，两个灯一起熄灭，不再打扰。
以下动作会重新点亮并再续十分钟：上电/重启、恢复出厂、开始重新配对。休眠期间
按一下遥控器按键，**D4 会短暂亮一下**表示"我听到了"，但**不会**重置那十分钟
的计时。网页上的提示不受指示灯休眠影响，会一直显示——指示灯的职责只是把
你叫过来看一眼。
...
`wifi join` 之外还有一条**免装软件**的路子：板子实现了 **Improv Wi-Fi 串口协议**，
所以可以直接用公开网页配网（见下节），不需要串口终端。
...
### 用公开网页配网（Improv 串口，免装软件）

板子实现了 **Improv Wi-Fi 串口协议**，所以配网可以完全不碰终端：用一个公开网页、
插着 USB 线点几下就行。用的是 **ESPHome Web / ESP Web Tools** 那一套现成页面，
本仓库不提供自己的配网页。

```
1. 用桌面版 Chrome / Edge 打开 https://web.esphome.io/
2. 点 Connect → 在端口列表里选板子那个口（COM3 / CH343）
3. 选 Wi-Fi、填密码 → 页面会自动跳到板子的配置页
```

要点：

- **必须是桌面版 Chrome / Edge（或 Opera）**，Web Serial 只有这些浏览器有；
  国产浏览器（QQ / 百度 / UC / 360）和 iOS 全部不支持。页面本身必须在 HTTPS
  或 localhost 下加载，`file://` 会被 CORS 挡掉。
- 协议和串口控制台**共用同一个 USB 口**：解析器只认领自己帧里的字节，其余原样
  交给控制台，所以配网时 `status` 之类的命令照常能用。
- 配网成功后网页拿到的是板子返回的 `http://<ip>/`，也就是**上面那个配置页**。
  那个页面还有自己的访问密码（见下节），和配网是两件事。
- 板子**每次开机都会广播**，配网失败可以重来；20 秒连不上会回到"可再次接收凭据"
  状态，不会卡在"连接中"。

> ⚠️ **已知冲突（需实测确认）**：本板把 USB 串口的 **DTR 接到了 GPIO9**，也就是
> BOOT 键。浏览器打开串口时 Windows 默认会置位 DTR，这**看起来和按住 BOOT 完全
> 一样**。固件对此做了防护：一旦收到过 Improv 帧，本次按住就不再触发 5 秒恢复
> 出厂（串口 `factory` 命令不受影响）。如果实测发现浏览器一连上板子就重启或
> 掉进下载模式，那就是这条硬件接线的问题，不是协议实现的问题。
...
115200 波特率，输入 `help` 看全表。**这个口同时跑 Improv 配网协议**（见上节），
两者互不干扰：二进制帧被解析器认领，文本命令照常执行。常用：
