...
3. **首页**查看遥控器 / 主机连接、最近上报电量与自定义映射数量；
   打开**按键映射**，点卡片或遥控器示意上的按键 → 选**键盘快捷键**或
...
新版为全离线、无外部字体 / 图片 / CDN 的首页、映射与设备信息三页布局，适配手机。
不提供音频、宏、双击 / 长按独立动作或全局映射启停开关；蓝色高亮表示编辑选中，
并非实体按键实时状态。**新版仅编译验证与本地 Chromium 模拟 API 回归；尚未烧录验证。**

无设备预览（明确标注模拟数据，不访问真实桥接器）：
